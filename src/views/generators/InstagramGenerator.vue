<template>
    <div class="space-y-4">
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('forms.igUsername') }}</label>
            <div class="relative">
                <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-gray-500 font-medium">@</span>
                <input type="text" v-model="username" placeholder="yourprofile"
...
const updateQr = () => {
    ...
    qrState.data = user ? `https://instagram.com/${user}` : 'https://instagram.com/'
}
